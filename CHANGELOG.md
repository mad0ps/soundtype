...
* **Живая расшифровка.** Текст появляется по ходу речи, а не только после
  остановки. Новый модуль `py/streaming.py`: `Segmenter` режет поток на фразы
  прямо в цикле записи, `DecodeWorker` распознаёт их в отдельном потоке.
  Событие `partial` показывает промежуточный результат.

  Это возвращение к потоковому режиму, от которого отказались в 0.4.0 — но
  сделанное правильно. Тогда распознавание вызывалось **из самого цикла
  чтения микрофона** и блокировало его; теперь оно в отдельном потоке, а цикл
  занят только чтением. Плюс обойдена ловушка sherpa-onnx #2918: декодируется
  только сегмент, никогда весь накопленный буфер.

* **Докачка модели из самого приложения.** Новый `py/downloader.py`: если
  модели нет, приложение показывает экран с кнопкой «Скачать» и прогрессом
  вместо того, чтобы молча не работать. События `deps-missing`,
  `download-progress`, `download-done`, `download-error`.

  Это снимает главное препятствие для публикации в OpenStore: раньше 660 МБ
  модели надо было ставить скриптом руками.

* `scripts/fetch-deps.py` стал тонкой обёрткой над `py/downloader.py` — один
  и тот же код зовут и командная строка, и кнопка в приложении.
* В права приложения добавлена группа `networking` — нужна для докачки.
